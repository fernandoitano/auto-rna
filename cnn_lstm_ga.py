# -*- coding: utf-8 -*-
"""
Biblioteca desenvolvida na tese de doutorado Auto RNA: Desenvolvimento 
automático de redes neurais utilizando algoritmos genéticos.
Autor: Fernando Itano

A primeira versão deste código foi desenvolvida em conjunto com 
Eder Fernando Urbinate, e os resultados obtidos foram publicados no ICAISC 2023
com o artigo 'CNN-LSTM Optimized by Genetic Algorithm in Time Series 
Forecasting: An Automatic Method to use Deep Learning'
"""
...
def main():
    ...
    # Sunspots
    dataset = 'monthly-sunspots.csv'
    file = f'data/{dataset}'
    ga.log_message(f'Loading DataSet: {file}')
    data = genfromtxt(file, delimiter=',')
    data = data[1:len(data)-1,1]

    # SCI
    # dataset = 'SCI000001.csv'
    # file = f'data/{dataset}'
    # ga.log_message(f'Loading DataSet: {file}')
    # data = genfromtxt(file, delimiter=',')
    # data = data[1:len(data)-1,1]
    ...
    dir_base = f'ensaios/{dataset}'
    exec_timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    dir_save = dir_base + '/cnn_models_' + exec_timestamp
    if not os.path.exists(dir_save):
        os.makedirs(dir_save)
    print(f'Directory: {dir_save}')
    #os.mkdir(dir_save)
    ...
    for generation in range(generations):
        ...
        for i, chromosome in enumerate(population):
            ...
            if isinstance(existing_chromosome, tuple):
                ...
                cnn_type = 'new cnn'
                if model != None:
                    model_name = dir_save + '/model_gen_' + str(generation).zfill(2) + '_cnn_' + str(i).zfill(2) + '.h5'
        ...
        ga_evolution_pd.to_csv(dir_save + '/ga_evolution_' + exec_timestamp + '.csv', mode='w', index=False)
...
if __name__ == "__main__":
    for i in range(1,2):
        try:
            ...
            print(f'Error in evolutionary process: {e}')
